...
void PhiMatrixOperations::RetrieveExternalTopicModel(const PhiMatrix& phi_matrix,
  ...
  if (get_model_args.token_size() > 0) {
    ...
    for (int i = 0; i < get_model_args.token_size(); ++i) {
      ...
      if (use_class_id_as_transaction) {
        ...
        transaction_type = TransactionType(get_model_args.transaction_type(i).value());
      }
    ...
    for (const auto& tt : get_model_args.transaction_type()) {
      transaction_types.insert(TransactionType(tt.value()));
    }
  ...
  for (int token_index : tokens_to_use) {
    ...
    auto ptr = topic_model->add_transaction_type();
    for (const ClassId& class_id : current_token.transaction_type.AsVector()) {
...
void PhiMatrixOperations::ApplyTopicModelOperation(const ::artm::TopicModel& topic_model,
  ...
  for (int token_index = 0; token_index < topic_model.token_size(); ++token_index) {
    ...
    if (topic_model.transaction_type_size() > 0) {
      transaction_type = TransactionType(topic_model.transaction_type(token_index).value());
    } else {
      transaction_type = TransactionType({ class_id });
    }
...

    {
      ...
      if (relative_reg) {
        ...
        if (regularizer->class_ids_to_regularize().size() > 0) {
          ...
          // ToDo(MelLain): STUB ONLY
          for (const ClassId& class_id : class_ids) {
            n_t_all.insert(std::make_pair(NormalizerKey(class_id, TransactionType({ class_id })),
                                          std::vector<float>()));
          }
        }
          ...
          if (iter != n_t_all.end()) {
            ...
            for (int topic_id = 0; topic_id < topic_size; ++topic_id) {
              ...
              for (int token_id = 0; token_id < token_size; ++token_id) {
                // ToDo(MelLain): STUB ONLY (iter->first.class_id())

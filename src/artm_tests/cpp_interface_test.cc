...
TEST(CppInterface, BasicTrasactionTest) {
  ...
  try {
    ...
    auto test_func = [](artm::MasterModel& master_component, const artm::InitializeModelArgs& init_model_args) {
      auto func = [](const artm::TopicModel& model, int num_transactions) {
        ...
        for (int i = 0; i < model.token_size(); ++i) {
          ...
          normalizer_keys[{ model.class_id(i), model.token_transaction_type_index(i) }] += value;
        }
      ...
      ASSERT_EQ(model_1.token_transaction_type_index_size(), model_1.token_size());
      ASSERT_EQ(model_1.all_transaction_types_size(), 4);
      func(model_1, 7);

      args.set_model_name(init_model_args.model_name());
      auto ptr = args.add_all_transaction_types();
      ...
      args.set_model_name(init_model_args.model_name());
      ptr = args.add_all_transaction_types();
      ptr->add_value("class_1");
      ptr->add_value("class_2");
    ...
    new_topic_model.add_token_transaction_type_index(0);
    new_topic_model.add_token("token_y");
    new_topic_model.add_class_id("class_5");
    new_topic_model.add_token_transaction_type_index(0);

    auto ptr = new_topic_model.add_all_transaction_types();
...

    {
      artm::GetTopicModelArgs args;
      args.set_model_name(new_topic_model.name());
      args.add_token("token_x");
      args.add_class_id("class_5");
      args.add_token_transaction_type_index(0);
      args.add_token("token_y");
      args.add_class_id("class_5");
      args.add_token_transaction_type_index(0);
      ptr = args.add_all_transaction_types();
      ptr->add_value("class_5");
